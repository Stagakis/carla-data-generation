...
def save_lidar(filename, filename_ply, lidar_measurement):
    #points = np.frombuffer(lidar_measurement.raw_data, dtype=np.float32).reshape([-1, 3])

    points = np.frombuffer(lidar_measurement.raw_data, dtype=np.dtype('f4')).reshape([-1, 3])
    ...
    # save_ply
    lidar_measurement.save_to_disk(filename_ply)
    #
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            #Stag's Edit
            spawn_point.location.x =  193.8
            spawn_point.location.y =  139.4
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            if(pygame.key.get_pressed()[K_b]):
                print("Trying to save pointcloud")
                save_lidar("test", "test.ply", image)
